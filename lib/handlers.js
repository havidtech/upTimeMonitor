...
handlers._tokens.get = (data, callback)=>{
    ...
    var id = typeof(data.queryStringObject.get('id')) == 'string' && data.queryStringObject.get('id').trim().length == 20 ? data.queryStringObject.get('id').trim() : false;
    if(id){
        // Lookup the token
    ...
    if(protocol && url && method && successCodes && timeoutSeconds){
        ...
        _data.read('tokens', token, (err,tokenData)=>{
            if(!err && tokenData){
                ...
                callback(400);
                console.log("Error reading user token",token);
            }
        })
    }else{
           ...
           // Lookup the check
            _data.read('checks',id,function(err,checkData){
                if(!err && checkData){
                    ...
                    callback(400,{'Error' : 'Specified check does not exist'});
                }
            });
        }else{
            callback(400,{'Error' : 'Missing fields to update.'});
                            ...
                            _data.read('users',checkData.userPhone,function(err,userData){
                                if(!err && userData){
                                    ...
                                    if(checkPosition > -1){
                                        ...
                                        callback(500,{'Error' : 'Could not find the check on the user\'s object and so could not remove it'});
                                    }
                                } else {
                                callback(400,{'Error' : 'Could not find the user who created the check and so could not remove the deleted check from the user object.'});
                                }
                            });  
                        } else {
